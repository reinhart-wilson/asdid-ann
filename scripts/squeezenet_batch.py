...
from tensorflow.keras import optimizers, metrics, callbacks, layers
...
lr          = 1e-4
batch_size  = 16*5
dense       = 256*(2**3)# Banyak unit di dense layer
...
model.build_model(include_classification_head=False)
dense_layer = layers.Dense(dense, activation='relu')
classification_head = model.get_classification_head()
model.add_layers([dense_layer, classification_head])
